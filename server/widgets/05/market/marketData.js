...
class MarketData {
  ...
  async getIndividualStock(symbol) {
    ...
    const response = await fetch(
      `${this.baseURLTD}/quote?symbol=${symbol}&apikey=${this.apiKeyTD}`
    );
  ...
  async getFeaturedStocks() {
    ...
    if (
      !mostRecentStockEntry ||
      Date.now() - mostRecentStockEntry.lastUpdated > this.cacheDurationDay
    ) {
  ...
  async updateFeaturedStocks() {
    const response = await fetch(
      `${this.baseURLAV}?function=TOP_GAINERS_LOSERS&apikey=${this.apiKeyAV}`
    );
    const data = await response.json();
  ...
  async getCompanyInfo(symbol) {
    ...
    const response = await fetch(
      `${this.baseURLAV}?function=OVERVIEW&symbol=${symbol}&apikey=${this.apiKeyAV}`
    );
    const data = await response.json();

    const logoURL =
      data.OfficialSite && data.OfficialSite !== 'None'
  ...
  async getStockWeeklyData(symbol) {
    ...
    const isDataStale = weeklyData && Date.now() - weeklyData.lastUpdated > this.cacheDurationWeek;
    ...
    const response = await fetch(
      `${this.baseURLTD}/time_series?symbol=${symbol}&interval=1day&outputsize=7&apikey=${this.apiKeyTD}`
    );
    const data = await response.json();
